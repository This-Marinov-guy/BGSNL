...
export default function HelpSection({ currentUser }) {
  return <div className="tab-content-wrapper"><div className={`tab-body ${styles.helpSection}`}>
    ...
    <nav className={styles.quickLinks} aria-label="Account help"><a href="/user#settings">Membership & billing</a><a href="/user#tickets">Your tickets</a><a href="/contact">Contact the society</a>{currentUser.status === "active" && currentUser.roles?.some((role) => SUPPORT_ACCESS.includes(role)) && <a href="/user/dashboard/support">Support inbox</a>}</nav>
