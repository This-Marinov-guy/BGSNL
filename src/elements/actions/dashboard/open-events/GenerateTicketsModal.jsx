...
const GenerateTicketsModal = ({ visible, onHide, event }) => {
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      const { ticketBlob } = await createCustomerTicket(event.ticketImg, element.name, element.surname, event.ticketColor, qrCode, event.ticketName);
  ...
  return (
    <Dialog header='Generate Free Guest Tickets'
      visible={visible}
      style={{ maxWidth: '90%' }}
      onHide={onHide}
      dismissableMask>
      ...
      <div className="mt--40 center_div">
        {loading ? <CustomSpinner /> : <>
          <button
            onClick={onHide}
            className="rn-button-style--2 rn-btn-reverse mr--5">Cancel</button>
          <button
            disabled={loading}
            type="submit"
