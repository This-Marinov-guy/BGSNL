...
export default function PaymentResultPreview({ outcome = "success", free = false }) {
  ...
  useEffect(() => {
    setDemoReady(false);
    if (outcome !== "account-preparing") return;
    const timer = window.setTimeout(() => setDemoReady(true), 8000);
    return () => window.clearTimeout(timer);
  }, [outcome]);
  const membership = ["account-preparing", "account-ready"].includes(outcome);
  const status = ["success", "cancelled", "failed", "expired", "processing", "unavailable"].includes(outcome) ? outcome : "success";
  return <PaymentResult autoSync={false} key={`${outcome}-${free}`} checkout={"0".repeat(24)} unavailable={status === "unavailable"}
      result={status === "unavailable" ? undefined : { status, kind: membership ? "subscription" : free ? "free" : "ticket", accountReady: outcome === "account-ready" || demoReady, isSignup: membership, amount: free ? 0 : 1200, currency: "eur", date: "2026-09-10T12:00:00Z",
        items: [{ description: membership ? "BGSNL membership" : "Bulgarian Dinner — Groningen", quantity: 1, amount: free ? 0 : 1200 }], reference: "BGSNL-PREVIEW",
